...
export default function espresso(pi: Host) {
  let enabled = true;
  let rtk = false;
  pi.registerCommand('espresso', {
    description: 'Concise output: on, off, status, rtk-on, rtk-off, ladder',
    getArgumentCompletions: prefix => ['on', 'off', 'status', 'rtk-on', 'rtk-off', 'ladder', 'worker ']
    ...
    handler: async (args, ctx) => {
      ...
      else if (!['status', 'ladder', 'worker'].includes(mode)) throw new Error('Use on, off, status, rtk-on, rtk-off, ladder or worker <assignment>.');
      ...
      notify(JSON.stringify({enabled, rtk, current, worker: target,
        routing: 'Named agents or /espresso worker <text assignment>; existing overrides preserved'}));
    },
  });
